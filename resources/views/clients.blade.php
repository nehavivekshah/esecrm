        ...
        <div class="container-fluid">
            <div class="board-title board-title-flex">
                ...
                @if(in_array('clients_add', $roleArray) || in_array('All', $roleArray))
                    <div class="btn-group">
                        ...
                        <a href="/manage-client" class="btn btn-primary btn-sm"><i class="bx bx-plus"></i> <span>Add
                                New</span></a>
                    </div>
                @endif
            </div>
            <div class="row">
                <div class="col-md-12 py-3 table-responsive">
                    <table id="lists" class="table table-condensed m-table clients"
                        ...
                        <tbody>
                            @foreach($clients as $client)
                                ...
                                <tr class="view selectrow" id="{{ $client->id ?? '' }}">
                                    ...
                                    <td>{{ $client->name ?? '' }}<span class="small d-none">{{ $client->company ?? '' }}</span>
                                    </td>
                                    ...
                                    <td class="m-none text-center">
                                        {!! date_format(date_create($client->created_at ?? ''), 'd M, Y') !!}</td>
                                    <td width="80px">@if($client->status == '1')<span
                                    class="badge bg-success">Active</span>@else<span
                                            class="badge bg-danger">Deactive</span>@endif</td>
                                    <td class="position-sticky end-0">
                                        <div class="table-btn m-none">
                                            @if(!empty($client->whatsapp))<a
                                                href="https://api.whatsapp.com/send/?phone={{ $client->whatsapp }}&text=Hi&type=phone_number&app_absent=0"
                                                class="btn btn-success bg-success text-white btn-sm" title="whatsapp"><i
                                            class="bx bxl-whatsapp"></i></a>@endif
                                            @if(!empty($client->email))<a href="mailto:{{ $client->email }}"
                                                class="btn btn-info text-white btn-sm" title="Email"><i
                                            class="bx bx-envelope"></i></a>@endif
                                            @if(!empty($client->mob))<a href="tel:{{ $client->mob }}"
                                                class="btn btn-warning text-dark btn-sm" title="Call"><i
                                            class="bx bx-phone"></i></a>@endif
                                            @if(in_array('client_edit', $roleArray) || in_array('All', $roleArray))
                                                <a href="/manage-client?id={{ $client->id ?? '' }}"
                                                    class="btn btn-primary text-white btn-sm edit" title="Edit"><i
                                            ...
                                            @if(in_array('client_delete', $roleArray) || in_array('All', $roleArray))
                                                <a href="javascript:void(0)" class="btn btn-danger btn-sm delete"
                                                    id="{{ $client->id }}" date-page="clientDelete" title="Delete"><i
                                        ...
                                        <div class="table-btn d-none">
                                            @if(!empty($client->whatsapp))<a
                                                href="https://api.whatsapp.com/send/?phone={{ $client->whatsapp }}&text=Hi&type=phone_number&app_absent=0"
                                                target="_blank" class="btn btn-success bg-success text-white btn-sm"
                                            title="whatsapp"><i class="bx bxl-whatsapp"></i></a>@endif
                                            @if(!empty($client->email))<a href="mailto:{{ $client->email }}"
                                                class="btn btn-info text-white btn-sm" title="Email"><i
                                            class="bx bx-envelope"></i></a>@endif
                                            @if(!empty($client->mob))<a href="tel:{{ $client->mob }}"
                                            ...
                                            class="bx bx-phone"></i></a>@endif
                                            @if(in_array('client_edit', $roleArray) || in_array('All', $roleArray))
                                                <a href="javascript:void(0)" class="btn btn-primary text-white btn-sm edit"
                                                    data-view-id="{{ $client->id ?? '' }}" title="Call"><i
                                                        class="bx bx-pencil"></i></a>
                                            @endif
                                            @if(in_array('client_delete', $roleArray) || in_array('All', $roleArray))
                                                <a href="javascript:void(0)" class="btn btn-danger btn-sm delete"
