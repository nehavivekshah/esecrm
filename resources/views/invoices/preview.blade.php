...
<style>
    ...
    .pdf-preview-canvas .text-right { text-align: right; }
    .pdf-preview-canvas .text-center { text-align: center; }
    .pdf-preview-canvas .text-left { text-align: left; }
    .pdf-preview-canvas h4, .pdf-preview-canvas strong { margin-bottom: 0px !important; color: #000; }
</style>
    ...
    <div class="container-fluid" style="max-width: 1000px; margin: 0 auto;">
        ...
        <div class="d-flex align-items-center justify-content-between bg-white px-4 py-3 mb-4 shadow-sm" style="border-radius: 12px; position: sticky; top: 85px; z-index: 100; box-shadow: 0 4px 15px rgba(0,0,0,0.03)!important; border: 1px solid #f1f3f4;">
            <div class="d-flex align-items-center gap-3">
                <a href="{{ url()->previous() == url()->current() ? '/invoices' : url()->previous() }}" class="btn btn-light" style="width: 40px; height: 40px; border-radius: 50%;" title="Back">
                    <i class="bx bx-arrow-back" style="font-size: 1.2rem; margin-top: 4px;"></i>
                </a>
                <div>
                    <h5 class="mb-0" style="font-weight: 700; color: #202124;">Invoice: INV-{{ $invoice->invoice_number }}</h5>
                </div>
            </div>
            <a href="/invoices/download/{{ $invoice->id ?? 0 }}" class="btn text-white px-4 py-2" style="background: linear-gradient(135deg, #dc3545, #b02a37); border-radius: 8px; font-weight: 600;">
            ...
            <table class="layout-table">
                <tr>
                    <td style="width: 50%;">
                        ...
                        <div><strong>Due Date:</strong> {{ $invoice->due_date ? \Carbon\Carbon::parse($invoice->due_date)->format('M d, Y') : 'N/A' }}</div>
                    </td>
                    <td style="width: 50%; text-align: right;">
            ...
            <div class="w-100" style="background-color: #e5e7eb; border-radius: 5px;">
                <table class="total-table" style="width: 100%;">
                    <tr>
                        ...
                        <td class="text-right" style="padding-right: 21px; width: 120px;"><strong>{{ number_format($grandTotal, 2) }}</strong></td>
                    </tr>
                </table>
            </div>
