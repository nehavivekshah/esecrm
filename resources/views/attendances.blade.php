    ...
    <div class="dash-container">
            ...
            <div class="rv-stat-card">
                ...
                <div>
                    ...
                    <div class="rv-stat-num" style="color:#1a73e8; font-size:1.05rem;">
                        {{ sprintf('%02d:%02d', floor($wh), round(($wh - floor($wh)) * 60)) }}<span class="rv-stat-sub"> / {{ sprintf('%02d:%02d', floor($eh), round(($eh - floor($eh)) * 60)) }}</span>
                    </div>
        ...
        <div class="leads-toolbar mb-3">
            <div class="leads-toolbar-left">
                <form method="GET" id="filterForm" class="d-flex align-items-center gap-2 m-0 p-0" style="flex-wrap:wrap;">
                    @if($isAdmin)
                ...
                <table id="lists" class="leads-table" style="width:100%;">
                    <thead>
                        <tr>
                            ...
                            <th>Remarks</th>
                            @if($canManage)<th class="text-center position-sticky end-0 bg-white" style="width:80px; border-left:1px solid #f1f3f4; box-shadow:-4px 0 8px rgba(0,0,0,0.02); z-index:10;">Action</th>@endif
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($final as $r)
                        ...
                        <tr>
                            @if($isAdmin)
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <div class="lb-avatar-sm" style="background:rgba(0,102,102,0.1); color:#006666; border:none; font-weight:800;">
                                            {{ substr($r['user'] ?? 'U', 0, 1) }}
                                        </div>
                            ...
                            <td><small class="fw-500 text-muted">{{ ($r['type'] && $r['type'] !== '-') ? $r['type'] : '—' }}</small></td>
                            <td class="{{ $diff_class }}">
                            ...
                            <td class="position-sticky end-0 bg-white" style="border-left:1px solid #f1f3f4; box-shadow:-4px 0 8px rgba(0,0,0,0.02); z-index:9;">
                                <div class="d-flex align-items-center justify-content-center gap-1">
                                    {{-- Edit – opens modal --}}
                                    <button type="button"
                                            class="btn kb-action-btn open-attendance-modal"
                                            data-url="{{ $editUrl }}"
                                            title="Edit"
                                    ...
                                    <a href="javascript:void(0)"
                                       class="btn kb-action-btn attendance-delete"
                                       id="{{ $r['att_id'] }}"
                                       title="Delete"
...
<style>
    /* ── Page Layout ── */
    .dash-container { padding: 24px 24px 24px; }   /* top margin added */

    /* ── Stat Cards (6-col grid) ── */
    .rv-stat-row { display: grid; grid-template-columns: repeat(6, 1fr); gap: 12px; }
    @media (max-width: 1200px) { .rv-stat-row { grid-template-columns: repeat(3, 1fr); } }
    @media (max-width: 768px)  { .rv-stat-row { grid-template-columns: repeat(2, 1fr); } }
    @media (max-width: 480px)  { .rv-stat-row { grid-template-columns: 1fr; } }
    ...
    .rv-stat-card:hover { box-shadow: 0 4px 16px rgba(0,0,0,0.07); border-color: #d2d4d7; transform: translateY(-1px); }
    .rv-stat-icon { width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.3rem; flex-shrink: 0; }
    .rv-stat-num  { font-size: 1.2rem; font-weight: 800; color: #202124; line-height: 1; white-space: nowrap; }
    .rv-stat-sub  { font-size: 0.7em; font-weight: 500; color: #80868b; }
    .rv-stat-label { font-size: 0.7rem; color: #80868b; margin-top: 4px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.3px; }

    /* ── Status Pills ── */
    .rv-status-pill { display: inline-flex; align-items: center; gap: 4px; border-radius: 20px; padding: 4px 10px; font-size: 0.72rem; font-weight: 700; white-space: nowrap; }
    .rv-status-pill i { font-size: 0.88rem; }

    /* ── Empty State ── */
    .rv-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px; color: #9aa0a6; text-align: center; gap: 8px; }
    .rv-empty i { font-size: 3rem; color: #dadce0; }
    .rv-empty span { font-size: 0.95rem; font-weight: 500; }

    /* ── Toolbar ── */
    .leads-toolbar { display: flex; align-items: center; justify-content: space-between; background: #fff; border: 1px solid #e8eaed; border-radius: 12px; padding: 8px 16px; min-height: 56px; flex-wrap: wrap; gap: 8px; }
    ...
    /* ── Filter Selects ── */
    .att-select { min-width: 140px; border-radius: 8px !important; cursor: pointer; transition: border-color .15s, box-shadow .15s; }
    .att-select:focus { border-color: #006666 !important; box-shadow: 0 0 0 3px rgba(0,102,102,0.1) !important; }

    /* ── Action Buttons ── */
    .kb-action-btn { width: 30px; height: 30px; border-radius: 8px !important; display: inline-flex; align-items: center; justify-content: center; font-size: 1rem; padding: 0; transition: all 0.15s; }
    .kb-action-btn:hover { transform: scale(1.12); }

    /* ── Record count badge ── */
    .lb-page-count { display: inline-flex; align-items: center; gap: 5px; background: rgba(0,102,102,0.08); color: #006666; border-radius: 20px; padding: 3px 10px; font-size: 0.75rem; font-weight: 600; }
</style>
