    ...
    <div class="dash-container">
                ...
                <div>
                    <div class="rv-stat-num" style="color:#1a73e8; font-size:1.05rem;">
                        @php
                            ...
                            echo sprintf('%02d:%02d', floor($wh), round(($wh - floor($wh)) * 60));
                        @endphp
                        <span class="rv-stat-sub"> / {{ sprintf('%02d:%02d', floor($eh), round(($eh - floor($eh)) * 60)) }}</span>
                    </div>
        ...
        <div class="leads-toolbar mb-3">
            ...
            <div class="leads-toolbar-right gap-2">
                ...
                @if($canManage)
                    <a href="/manage-attendance" class="lb-btn lb-btn-primary">
                ...
                <table id="lists" class="leads-table" style="width:100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><small class="fw-500 text-muted">{{ $r['type'] ?: '—' }}</small></td>
                            <td class="{{ $diff_class }}">
                            ...
                            <td class="position-sticky end-0 bg-white" style="border-left:1px solid #f1f3f4; box-shadow:-4px 0 8px rgba(0,0,0,0.02); z-index:9;">
                                <div class="d-flex align-items-center justify-content-center gap-1">
                                    <a href="/manage-attendance?user_id={{ $r['user_id'] }}&date={{ $r['date'] }}"
                                       class="btn kb-action-btn" title="Edit"
                                       style="background:rgba(0,102,102,0.10);color:#006666; border:none;">
                                        <i class="bx bx-edit"></i>
                                    </a>
                                    @if(isset($r['att_id']) && $r['att_id'])
                                    <a href="javascript:void(0)"
                                       class="btn kb-action-btn kb-action-del attendance-delete"
                                       id="{{ $r['att_id'] }}"
                                       data-page="attendanceDelete"
                                       title="Delete"
                        ...
                        <tr>
                            <td colspan="{{ ($isAdmin ? 1 : 0) + ($canManage ? 1 : 0) + 9 }}" class="text-center py-5">
                                <div class="rv-empty">
                                    ...
                                    @if($canManage)
                                        <a href="/manage-attendance" class="lb-btn lb-btn-primary mt-2" style="font-size:0.82rem;">
...
<style>
    ...
    .rv-empty {
        ...
        justify-content: center; padding: 40px; color: #9aa0a6;
    ...
    .leads-toolbar {
        ...
        border-radius: 12px; padding: 8px 16px; min-height: 56px;
    ...
    /* ── Filter Selects ── */
    .att-select {
        min-width: 140px; border-radius: 8px !important;
        cursor: pointer;
...
<script>
    ...
    function exportAttendanceCSV() {
        ...
        rows.forEach(function (row) {
            ...
            // Remove last item (Action column)
            const actionIndex = rowData.length - 1;
            // Only remove if we have the actions column rendered
            const hasAction = {{ $canManage ? 'true' : 'false' }};
            if (hasAction) rowData.pop();
        ...
        a.href     = url;
        a.download = 'attendance_export_{{ now()->format("Y-m-d") }}.csv';
        a.click();
