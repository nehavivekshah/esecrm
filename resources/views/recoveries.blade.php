                        ...
                        <tr class="lead-row-{{ $reminderTimes }}">
                            ...
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-muted" style="font-size:0.72rem;">{{ $recovery->company ?? '' }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="position-sticky end-0 bg-default">
                                <div class="d-flex align-items-center justify-content-center gap-1">
                                    {{-- Reminder --}}
                                    <button class="kb-action-btn reminder"
                                            data-id="{{ $recovery->id ?? '' }}"
                                            title="Set Reminder"
                                    ...
                                    {{-- Mark Received --}}
                                    <button class="kb-action-btn received"
                                            data-id="{{ $recovery->id ?? '' }}"
                                            title="Mark Received"
                                    ...
                                    @if(!empty($recovery->whatsapp))
                                        <a href="https://api.whatsapp.com/send/?phone={{ $recovery->whatsapp }}&text=Hi&type=phone_number&app_absent=0"
                                           class="kb-action-btn" target="_blank" title="WhatsApp"
                                    ...
                                    @if(!empty($recovery->mob))
                                        <a href="tel:+{{ $recovery->mob }}"
                                           class="kb-action-btn" title="Call"
                                           style="background:rgba(26,115,232,0.10);color:#1a73e8;">
                                            <i class="bx bx-phone"></i>
                                        </a>
                                    @endif
                                    {{-- Edit --}}
                                    <a href="/manage-recovery?id={{ $recovery->id ?? '' }}"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content" style="border-radius:16px; border:none;">
            <div class="modal-header" style="border-bottom:1px solid #f0f0f0; padding:16px 20px;">
                <h5 class="modal-title" id="recoveryModalLabel" style="font-size:0.95rem; font-weight:700; color:#202124;">
                    Recovery Details
                </h5>
...
.rv-overdue-pill {
    ...
    color: #ea4335; border-radius: 20px; padding: 3px 10px;
    font-size: 0.73rem; font-weight: 600;
}

/* ── Batch badge ── */
.rv-batch {
    display: inline-block; background: #f1f3f4; border-radius: 6px;
    padding: 2px 8px; font-size: 0.72rem; font-weight: 600; color: #5f6368;
}

/* ── Amount ── */
.rv-amount { font-weight: 700; color: #202124; font-size: 0.875rem; }
.rv-overdue-amount { color: #ea4335; }
.rv-amount-badge {
    display: inline-flex; align-items: center; gap: 4px;
    font-size: 0.72rem; font-weight: 600; border-radius: 20px; padding: 3px 9px;
}
.rv-paid { background: rgba(52,168,83,0.08); color: #34a853; }
...
.rv-reminder i { font-size: 0.9rem; }
.rv-reminder-overdue { color: #ea4335; }
.rv-overdue-dot {
    background: rgba(234,67,53,0.10); color: #ea4335;
